...
const PLOT: f32 = 700.0;
const SPARK_H: f32 = 90.0;
...
async fn consume(
    ...
    let ticket = Ticket::new(format!("{{\"speed\": {speed}}}"));
...
/// Redraws when the feed task reports new data.
struct Redraw;
#[async_trait::async_trait]
impl EventStreamHandler<State> for Redraw {
    async fn handle(
        &self,
        _: &SceneGraphEvent,
        _: &mut State,
...
fn main() {
    ...
    let (width, height) = (PLOT + 190.0, PLOT + 230.0);
    ...
    if let Some(dir) = snapshots {
        data_rt.block_on(async move {
            ...
            let feed = tokio::spawn(consume(addr, speed, window_s, live.clone(), None));
            let mut state = state;
    ...
    data_rt.spawn(async move {
        if let Err(e) = consume(addr, speed, window_s, live, Some(feed_hub)).await {
    ...
    let app = window_rt
        .block_on(AvengerApp::try_new(
            ...
            vec![(
                ...
                Arc::new(Redraw) as Arc<dyn EventStreamHandler<State>>,
            )],
        ))
        .expect("build app");
